...
.continuereset
    ...
    sta.b $00
    ldy #$0015 ; 0n15 bytes
    lda.w #(!SRAM_MW_SM) ; copy to SRAM_MW_SM
    sta.b $03
    jsl copy_memory
    ...
    sta.b $00
    ldy #$0015 ; 0n15 bytes
    lda.w #(!SRAM_MW_ROMTITLE) ; copy to SRAM_MW_ROMTITLE
    sta.b $03
    jsl copy_memory
...
.own_item
    ...
    cmp #$001E
    bmi .own_item1
    lda #$0016              ; self item id >= #$001F should never happen... but just call it a small ammo tank (#$0019) to avoid a crash
.own_item1
...
.item_link_item
    ...
    lda.l rando_item_table+$2, x    ; X = original item id aka message table index again
    tax
    cmp #$001E ; } skip immediate pickup and message box if this isn't an SM item
